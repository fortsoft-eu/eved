...
function focusAdminElement(oElement, blSelectText) {
    ...
    try {
        oElement.focus({"preventScroll":true});
    } catch (oException) {
        oElement.focus();
    }
...
function findAdminSubjectRowById(sSubjectId) {
    return sSubjectId ? document.querySelector("#nx-subjects-table tbody tr[data-subject-id=\"" + sSubjectId + "\"], #nx-birthdays-table tbody tr[data-subject-id=\"" + sSubjectId + "\"], #nx-interactions-table tbody tr[data-subject-id=\"" + sSubjectId + "\"]") : null;
}
...
document.addEventListener("DOMContentLoaded", function () {
    ...
    function getEmptyRowCount() {
        var iCount = oRows ? parseInt(oRows.getAttribute("data-empty-row-count") || "1", 10) : 1;
        return isNaN(iCount) || iCount < 1 ? 1 : iCount;
    }
    function getRows() {
    ...
    function isComplexFilterSelectValueType(sValueType) {
        return sValueType == "boolean" || sValueType == "group" || sValueType == "address_type";
    }
...
document.addEventListener("click", function (oEvent) {
    ...
    function fallbackCopyValue(sText) {
        var oTextArea = document.createElement("textarea");
        var blResult = false;
        oTextArea.value = sText;
        oTextArea.setAttribute("readonly", "readonly");
    ...
    function showCopyValueResult(blSuccess) {
        ...
        if (oBox) {
            oBox.textContent = blSuccess ? "\u2714\uFE0F" : "\u274C\uFE0F";
        } else {
            oButton.textContent = blSuccess ? "\u2714\uFE0F" : "\u274C\uFE0F";
        }
        window.setTimeout(function () {
            if (oBox) {
    ...
    if (navigator.clipboard && navigator.clipboard.writeText) {
        navigator.clipboard.writeText(sValue).then(function () {
            ...
            showCopyValueResult(fallbackCopyValue(sValue));
        });
        return;
    }
    showCopyValueResult(fallbackCopyValue(sValue));
...
document.addEventListener("DOMContentLoaded", function () {
    ...
    function openConfirmDialog(sAction, sUnassignId, sFilmRoll, sLabBag, oRow) {
        ...
        var oCsrfHidden = document.createElement("input");
        ...
        var sCsrfToken = getAdminCsrfToken();
        var closeOnEscape = function (oEvent) {
            if (oEvent.key == "Escape") {
                closeDialog();
            }
        ...
        oCsrfHidden.type = "hidden";
        oCsrfHidden.name = "ex_csrf_token";
        oCsrfHidden.value = sCsrfToken;
...
document.addEventListener("DOMContentLoaded", function () {
    ...
    function setGroupDialogError(oError, sMessage) {
        oError.textContent = sMessage || "";
        oError.style.display = sMessage ? "" : "none";
    }
    function refreshGroupFilter() {
        var oFilter = document.querySelector(".js-table-filter");
        var oEvent;
        if (!oFilter) {
            return;
        }
    ...
    function replaceGroupRow(iGroupId, sRowHtml) {
        ...
        if (oCurrentRow && (" " + oCurrentRow.className + " ").indexOf(" nx-admin-row-modal ") !== -1) {
    ...
    function submitGroupDialog(oDialogData, oData) {
        setGroupDialogError(oDialogData.error, "");
        ...
        fetch(window.location.href, {
            ...
            if (!aData || !aData.success) {
                setGroupDialogError(oDialogData.error, aData && aData.message ? aData.message : "Group could not be saved.");
            ...
            setGroupDialogError(oDialogData.error, "Group could not be saved.");
            oDialogData.save.disabled = false;
        });
    }
    ...
    function openGroupMergeDialog(oRow) {
        ...
        oDialogData.form.addEventListener("submit", function (oEvent) {
            ...
            if (iSelectedCount < 1) {
                setGroupDialogError(oDialogData.error, "Select at least one source group.");
                return;
            }
...
document.addEventListener("DOMContentLoaded", function () {
    ...
    function setContactTypeDialogError(oError, sMessage) {
        oError.textContent = sMessage || "";
        oError.style.display = sMessage ? "" : "none";
    }
    function refreshContactTypeFilter() {
        var oFilter = document.querySelector(".js-table-filter");
        var oEvent;
        if (!oFilter) {
            return;
    ...
    function closeContactTypeDialog(oDialog) {
        if (oDialog && oDialog.parentNode) {
            oDialog.parentNode.removeChild(oDialog);
            unlockAdminModalScroll();
        }
    ...
    function createContactTypeDialog(sTitle, oRow) {
        ...
        var closeDialog = function (blSaved) {
            ...
            closeContactTypeDialog(oDialog);
        };
        oDialog.className = "confirm-dialog";
    ...
    function submitContactTypeDialog(oDialogData, oData) {
        setContactTypeDialogError(oDialogData.error, "");
        oDialogData.save.disabled = true;
        appendAdminCsrfToken(oData);
        fetch(window.location.href, {
            "method": "POST",
            ...
            if (!aData || !aData.success) {
                setContactTypeDialogError(oDialogData.error, aData && aData.message ? aData.message : "Contact type could not be saved.");
                oDialogData.save.disabled = false;
                return;
            }
            if (aData.rows_html) {
            ...
            setContactTypeDialogError(oDialogData.error, "Contact type could not be saved.");
            oDialogData.save.disabled = false;
        });
    }
    ...
    function openContactTypeMergeDialog(oRow) {
        ...
        oDialogData.form.addEventListener("submit", function (oEvent) {
            ...
            if (iSelectedCount < 1) {
                setContactTypeDialogError(oDialogData.error, "Select at least one source contact type.");
                return;
            }
...
document.addEventListener("DOMContentLoaded", function () {
    ...
    function getSubjectCountryOptions() {
        ...
        if (window.Intl && typeof window.Intl.DisplayNames == "function") {
            try {
                oNames = new window.Intl.DisplayNames([sLanguage || "en"], {"type":"region"});
            } catch (oException) {
                oNames = null;
            }
        }
        for (iI = 0; iI < aSubjectCountryCodes.length; iI += 1) {
            ...
            aOptions.push({"code":aSubjectCountryCodes[iI], "name":sName || aSubjectCountryCodes[iI]});
        }
    ...
    function setSubjectDialogError(oError, sMessage) {
        oError.textContent = sMessage || "";
        oError.style.display = sMessage ? "" : "none";
    }
    ...
    function finishSubjectDialog(oDialogData, oFocus) {
        ...
        lockAdminModalScroll();
        beginAdminSubjectRowEdit(findAdminSubjectRowById(oDialogData.subjectId) || oDialogData.subjectRow);
        if (oFocus) {
            focusAdminElement(oFocus, true);
        }
    }
    function submitSubjectDialog(oDialogData, oData) {
        setSubjectDialogError(oDialogData.error, "");
        oDialogData.save.disabled = true;
        appendAdminCsrfToken(oData);
        fetch(window.location.href, {
            "method": "POST",
            ...
            if (!aData || !aData.success) {
                setSubjectDialogError(oDialogData.error, aData && aData.message ? aData.message : "Item could not be saved.");
                oDialogData.save.disabled = false;
                return;
            }
            if (aData.group) {
            ...
            setSubjectDialogError(oDialogData.error, "Item could not be saved.");
            oDialogData.save.disabled = false;
        });
    }
    ...
    function openSubjectDialog(aSubject, oSubjectRow, blNewSubject) {
        ...
        oDialogData.form.addEventListener("submit", function (oEvent) {
            ...
            if (oBirthNumber && blPerson && !isSubjectBirthNumberDigitCountValid(oBirthNumber.value)) {
                setSubjectDialogError(oDialogData.error, "Birth number must contain 9 or 10 digits.");
                oBirthNumber.focus();
                return;
            }
            var aPayload = {
    ...
    function openSubjectPortalDialog(aSubject, oSubjectRow) {
        ...
        setPortalFields = function () {
            ...
            setSubjectDialogError(oDialogData.error, blSupported ? "" : "Portal account can be set only for person or service.");
    ...
    function openAddressDialog(oItem, oSubjectRow, blNewAddress) {
        ...
        oDialogData.form.addEventListener("submit", function (oEvent) {
            ...
            if (oCountry._countryInput && oCountry._countryInput.value.replace(/^\s+|\s+$/g, "") === "") {
                setSubjectDialogError(oDialogData.error, "Country is required.");
                return;
            }
            if (oCountry.getAttribute("data-country-invalid") == "1") {
                setSubjectDialogError(oDialogData.error, "Invalid country.");
                return;
            }
    ...
    function openDeleteSubjectDialog(oButton) {
        openDeleteDialog("Confirm Deletion", "Delete this subject?", [
            ...
            {"name":"subject_id", "value":oButton.getAttribute("data-subject-id") || ""}
    ...
    function openDeleteContactDialog(oItem) {
        openDeleteDialog("Confirm Deletion", "Remove this contact from the subject?", [
            {"name":"action", "value":"delete_subject_contact"},
            {"name":"subject_contact_id", "value":getSubjectItemValue(oItem, "data-subject-contact-id")}
        ], getAdminSubjectRow(oItem));
    }
    function openDeleteNicknameDialog(oItem) {
        openDeleteDialog("Confirm Deletion", "Delete this nickname?", [
            {"name":"action", "value":"delete_subject_nickname"},
            {"name":"nickname_id", "value":getSubjectItemValue(oItem, "data-nickname-id")}
        ], getAdminSubjectRow(oItem));
    }
    function openDeleteAddressDialog(oItem) {
        openDeleteDialog("Confirm Deletion", "Delete this address?", [
            {"name":"action", "value":"delete_subject_address"},
            {"name":"address_id", "value":getSubjectItemValue(oItem, "data-address-id")}
        ], getAdminSubjectRow(oItem));
    }
    function openDeleteGroupDialog(oItem) {
        openDeleteDialog("Confirm Deletion", "Remove this group from the subject?", [
            {"name":"action", "value":"delete_subject_group"},
            {"name":"subject_id", "value":getSubjectItemValue(oItem, "data-subject-id")},
            {"name":"group_id", "value":getSubjectItemValue(oItem, "data-group-id")}
        ], getAdminSubjectRow(oItem));
    }
    function openDeleteNoteDialog(oItem) {
        openDeleteDialog("Confirm Deletion", "Delete this note?", [
            {"name":"action", "value":"delete_subject_note"},
            {"name":"note_id", "value":getSubjectItemValue(oItem, "data-note-id")}
        ], getAdminSubjectRow(oItem));
    }
    document.addEventListener("click", function (oEvent) {
...
document.addEventListener("DOMContentLoaded", function () {
    ...
    function closeContactDialog(oDialog) {
        if (oDialog && oDialog.parentNode) {
            oDialog.parentNode.removeChild(oDialog);
            unlockAdminModalScroll();
        }
    ...
    function getYouTubeContactHref(sValue) {
        ...
        blLooksLikeUrl = /^https?:\/\//i.test(sText)
            || /^www\./i.test(sText)
            || /^(youtube\.com|www\.youtube\.com)(?:[\/:?#].*)?$/i.test(sText)
            || /^[A-Za-z0-9.-]+\.[A-Za-z]{2,}[\/:?#].*$/i.test(sText);
        if (blLooksLikeUrl) {
            try {
    ...
    function isKnownExternalContactType(sType) {
        return sType == "web" || sType == "telegram" || sType == "messenger" || sType == "twitter"
            || sType == "mastodon" || sType == "bluesky" || sType == "threads" || sType == "facebook"
            || sType == "instagram" || sType == "tiktok" || sType == "linkedin" || sType == "github"
            || sType == "gitlab" || sType == "bitbucket" || sType == "stackoverflow" || sType == "deviantart"
            || sType == "furaffinity" || sType == "sofurry" || sType == "artstation" || sType == "behance"
            || sType == "dribbble" || sType == "youtube" || sType == "twitch" || sType == "kick"
            || sType == "vimeo" || sType == "reddit" || sType == "lemmy" || sType == "steam"
            || sType == "npm" || sType == "pypi" || sType == "docker" || sType == "codeberg"
            || sType == "paypal" || sType == "revolut" || sType == "orcid" || sType == "goodreads"
            || sType == "lastfm" || sType == "signaly";
    }
    ...
    function fallbackCopyContactValue(sText) {
        var oTextArea = document.createElement("textarea");
        var blResult = false;
        oTextArea.value = sText;
        oTextArea.setAttribute("readonly", "readonly");
    ...
    function copyContactValue(oButton) {
        ...
        if (navigator.clipboard && navigator.clipboard.writeText) {
            navigator.clipboard.writeText(sValue).then(function () {
                ...
                showContactCopyResult(oButton, fallbackCopyContactValue(sValue));
            });
            return;
        }
        showContactCopyResult(oButton, fallbackCopyContactValue(sValue));
    ...
    function setContactDialogError(oError, sMessage) {
        oError.textContent = sMessage || "";
        oError.style.display = sMessage ? "" : "none";
    }
    ...
    function updateContactElement(oItem, aContact) {
        ...
        if (oPrimary) {
            oPrimary.textContent = blIsPrimary ? "\u2B50" : "";
        }
        if (oInactive) {
            oInactive.textContent = blIsActive ? "" : "\u26D4";
        }
    }
    ...
    function openContactDialog(oItem, oSubjectRowParam, blNewContact) {
        ...
        oForm.addEventListener("submit", function (oEvent) {
            var oData = new FormData();
            oEvent.preventDefault();
            setContactDialogError(oError, "");
            oSave.disabled = true;
            if (!oType.value) {
                setContactDialogError(oError, "Select a contact type from the list.");
            ...
            fetch(window.location.href, {
                ...
                if (!aData || !aData.success) {
                    setContactDialogError(oError, aData && aData.message ? aData.message : "Contact could not be saved.");
                    oSave.disabled = false;
                    return;
                }
                if (aData.contact) {
                ...
                setContactDialogError(oError, "Contact could not be saved.");
                oSave.disabled = false;
            });
        });
        document.addEventListener("keydown", closeOnEscape);
