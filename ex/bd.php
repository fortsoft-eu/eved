...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST["action"]) && $_POST["action"] == "save_bd_settings") {
    ...
    $_SESSION["ex_bd_settings"] = removeCountrySettings($aBirthdaySettings);
    session_write_close();
    sendSecurityHeaders();
...
if ($sBdPostAction == "update_subject_portal_user") {
    ...
    try {
        ...
        saveSubjectPortalAccess($oPdo, $iSubjectId, $aSubjectRow["subject_type"], $aPayload);
        $oPdo->commit();
        sendJsonAndExit(bdGetUpdatedSubjectResponse($oPdo, $iSubjectId, $aBirthdaySettings, $blCanEdit));
    } catch (Exception $oException) {
        error_log((string)$oException);
        if ($oPdo->inTransaction()) {
            $oPdo->rollBack();
...
if ($sBdPostAction == "update_subject") {
    ...
    try {
        ...
        if ($sSubjectType != "" && $sSubjectType != $aSubjectRow["subject_type"]) {
            $oPdo->rollBack();
        ...
        if ($aSubjectRow["subject_type"] == "person" || $sSubjectName == "") {
        ...
        if ($aSubjectRow["subject_type"] != "person") {
...
if ($sBdPostAction == "update_subject_group") {
    ...
    try {
        ...
        $oStatement->execute(array("name" => $sGroupName, "id" => $iGroupId));
        $oPdo->commit();
        $aResponse = bdGetUpdatedSubjectResponse($oPdo, $iSubjectId, $aBirthdaySettings, $blCanEdit);
...
if ($sBdPostAction == "create_contact") {
    ...
    $sContactValue = normalizeContactInputForStorage($aContactType["contact_type"], $sContactValue);
    if ($sContactValue === false) {
        sendJsonAndExit(array("success" => false, "message" => contactInputErrorMessage($aContactType["contact_type"])), 400);
    }
    if ($sContactValue == "") {
...
if ($sBdPostAction == "update_contact") {
    ...
    try {
        ...
        $oPdo->commit();
        $aResponse = bdGetUpdatedSubjectResponse($oPdo, (int)$aUpdatedContact["subject_id"], $aBirthdaySettings, $blCanEdit);
        sendJsonAndExit($aResponse);
    } catch (Exception $oException) {
        error_log((string)$oException);
        if ($oPdo->inTransaction()) {
...
?>
    ...
    <span class="table-record-count js-table-record-count" data-table-count="birthdays-table" aria-live="polite"><?php echo count($aBirthdayRows); ?></span>
  </p>
...
<?php
    ...
    foreach ($aBirthdayRows as $aRow) {
        echo bdRenderSubjectRow($aRow, $aContacts, $aNicknames, $aAddresses, $aGroups, $aNotes, $blCanEdit, $aHiddenInactive, $aBirthdaySettings);
    }
    echo "    </tbody>\n",
        "  </table>\n";
}
